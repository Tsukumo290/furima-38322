class PurchaseHistoryOrder
  ...
  with_options presence: true do
    ...
    validates :item_id
    validates :post_code,
              format: { with: /\A[0-9]{3}-[0-9]{4}\z/, message: 'code is invalid. Enter it as follows (e.g. 123-4567)' }
    validates :prefecture_id, numericality: { other_than: 0, message: "can't be blank" }
    validates :municipality
    validates :address
    validates :phone_number, format: { with: /\A[０-９]{10,11}\z/, message: 'is too short' }
  end
  ...
  def save
    ...
    Order.create(post_code: post_code, prefecture_id: prefecture_id, municipality: municipality, address: address,
